...
constexpr unsigned TYPEID_PLAYER=4,TYPEID_UNIT=3;
struct Unit {unsigned type=4,map=821;bool walk=true,swim=true;bool CanWalk()const{return walk;}bool CanSwim()const{return swim;}unsigned GetTypeId()const{return type;}unsigned GetMapId()const{return map;}};
struct Config {bool enabled=true;bool GetBoolDefault(char const* key,bool fallback){return std::string(key)=="mmap.PlayerWalkable.821"?enabled:fallback;}} sConfig;
...
int main(){
 ...
 for(unsigned map:{0u,1u,30u,489u,529u}){u.map=map;p.createFilter();Check(p.m_filter.exclude==0);}u.map=821;
 ...
 std::cout<<"Native opt-in map/player slope filter, other maps, creature swimming and polygon lookup passed\n";
}
