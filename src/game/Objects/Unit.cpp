...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss, Spell* spell, bool addThreat, bool reflected)
{
    ...
    auto ShouldEnterCombat = [&]()
    {
        ...
        if (spellProto)
        {
            if (spellProto->HasAttribute(SPELL_ATTR_EX_NO_THREAT))
                return false;

            if (spellProto->HasAura(SPELL_AURA_DAMAGE_SHIELD))
                return false;
        }
...
void Unit::Kill(Unit* pVictim, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    if (spiritOfRedemptionTalentImmune)
    {
        ...
        int32 heal = pVictim->GetMaxHealth();
        pVictim->CastCustomSpell(pVictim, 27827, &heal, nullptr, nullptr, true);
    }
    else
    {
        pVictim->SetHealth(0);
    ...
    if (holder->IsSingleTarget())
    {
        if (Unit* caster = holder->GetCaster())             // caster not in world
        {
            ...
            for (SingleCastSpellTargetMap::iterator itr = scTargets.begin(); itr != scTargets.end();)
            {
                ...
                if (itr_targetGuid != GetObjectGuid() && Spells::IsSingleTargetSpells(itr_spellEntry, aurSpellInfo))
                {
...
bool Unit::RemoveNoStackAurasDueToAuraHolder(SpellAuraHolder *holder)
{
    ...
    uint32 idxCount = 0;
    const uint32 MaxCount = m_spellAuraHolders.size() * 2;
...
void Unit::RemoveSpellAuraHolder(SpellAuraHolder *holder, AuraRemoveMode mode)
{
    ...
    SpellEntry const* AurSpellInfo = holder->GetSpellProto();
    if (AurSpellInfo->IsChanneledSpell() && caster)
    ...
    if (mode != AURA_REMOVE_BY_EXPIRE && AurSpellInfo->IsChanneledSpell() && !AurSpellInfo->IsAreaOfEffectSpell() && caster)
        ...
        if (Player* pPlayer = ToPlayer())
        {
            if (SpellEntry const* pCreateBySpell = sSpellMgr.GetSpellEntry(pGo->GetSpellId()))
            {
                ...
                if (pCreateBySpell->HasAttribute(SPELL_ATTR_DISABLED_WHILE_ACTIVE))
                    ...
                    pPlayer->AddSpellAndCategoryCooldowns(pCreateBySpell, 0, nullptr, true);
            }
        }
    }
}
 ...
 */
bool Unit::NearTeleportTo(const WorldLocation location, const uint32_t teleportOptions)
{
    return NearTeleportTo(location.x, location.y, location.z, location.o, teleportOptions);
}
...
void Unit::ScheduleAuraStateRemoval(AuraState flag, uint32 delayMs)
{
    m_Events.AddLambdaEventAtOffset([this, flag]
    {
        ModifyAuraState(flag, false);
    }, delayMs);
}
...
bool Unit::IsImmuneToSpell(SpellEntry const* spellInfo, bool /*castOnSelf*/) const
{
    ...
    if (!spellInfo->HasAttribute(SPELL_ATTR_UNAFFECTED_BY_INVULNERABILITY)          // ignore invulnerability
        ...
        spellInfo->HasAuraOrTriggersAnotherSpellWithAura(SPELL_AURA_SCHOOL_IMMUNITY))
    {
        for (auto const& itr : m_spellAuraHolders)
        {
            ...
            if (pAuraSpell->HasAuraInterruptFlag(AURA_INTERRUPT_FLAG_UNK21))
                return true;
        }
    }
...
void Unit::SetInCombatWithAggressor(Unit* pAggressor, bool touchOnly/* = false*/)
{
    ...
    if (!touchOnly)
    {
        // in classic out of combat player hit by aoe from creature gets put in combat for 5 seconds
        SetInCombatState(UsesPvPCombatTimer() ? UNIT_PVP_COMBAT_TIMER : 0, pAggressor);
        if (Creature* pCreature = ToCreature())
            pCreature->UpdateLeashExtensionTime();

        if (Player* pOwner = ::ToPlayer(GetCharmerOrOwner()))
        {
            if (pOwner->IsTargetable(true, pAggressor->IsCharmerOrOwnerPlayerOrPlayerItself()) && !pOwner->IsFeigningDeathSuccessfully())
...
void Unit::SetInCombatWithVictim(Unit* pVictim, bool touchOnly/* = false*/, uint32 combatTimer/* = 0*/, bool direct/* = true*/)
{
    ...
    if (!touchOnly)
    {
        bool const victimIsPlayerOrPet = pVictim->UsesPvPCombatTimer();
...
bool Unit::HandleAttackPowerModifier(AttackPowerModIndex index, AttackPowerModType modifierType, float amount, bool apply)
{
    ...
    {
        sLog.outError("ERROR in HandleAttackPowerModifier(): nonexistent index %u or type %u!", index, modifierType);
        return false;
    }
...
float Unit::GetAttackPowerModifierValue(AttackPowerModIndex index, AttackPowerModType modifierType) const
{
    if (index >= AP_MODS_COUNT || modifierType >= AP_MOD_TYPE_COUNT)
    {
        sLog.outError("Attempt to access nonexistent modifier value from m_attackPowerMods!");
        return 0.0f;
    }
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* pTarget, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellEntry const* procSpell, uint32 damage, ProcTriggeredList& triggeredList, Spell* spell)
{
    ...
    for (const auto& itr : GetSpellAuraHolderMap())
    {
        ...
        if (itr.second->GetSpellProto()->HasAttribute(SPELL_ATTR_EX2_PROC_COOLDOWN_ON_FAILURE) && 
            HasSpellCooldown(itr.second->GetId()))
            continue;
        ...
        if (result != SPELL_PROC_TRIGGER_OK)
        {
            if (result == SPELL_PROC_TRIGGER_ROLL_FAILED &&
                ...
                AddSpellCooldown(itr.second->GetId(), 0, time(nullptr) + spellProcEvent->cooldown);
...
{
    if (!state && IsPlayer() && static_cast<Player*>(this)->HasChallenge(CHALLENGE_WAR_MODE))
        state = true;
...
void Unit::KnockBack(float angle, float horizontalSpeed, float verticalSpeed)
{
    ...
    if (IsMovedByPlayer() && IsInWorld())
    {
        ...
        if (Player* pPlayer = ToPlayer())
        {
            if (auto antiCheat = pPlayer->GetSession()->GetAntiCheat())
                antiCheat->KnockBack(horizontalSpeed, verticalSpeed, vcos, vsin);
        }
    }
}
