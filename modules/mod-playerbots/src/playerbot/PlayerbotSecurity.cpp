...
#ifdef MANGOSBOT_ONE
        if (GetBotAI(bot)->HasRealPlayerMaster() && bot->GetSession()->m_lfgInfo.queued)
#endif
...
bool PlayerbotSecurity::CheckLevelFor(PlayerbotSecurityLevel level, bool silent, Player* from, bool ignoreGroup)
{
    ...
    if (silent || (GetBotAI(from) && !GetBotAI(from)->IsRealPlayer()))
        return false;

    Player* master = GetBotAI(bot)->GetMaster();
    if (master && GetBotAI(bot) && GetBotAI(bot)->IsOpposing(master) && master->GetSession()->GetSecurity() < SEC_GAMEMASTER)
        return false;
    ...
    case PlayerbotSecurityLevel::PLAYERBOT_SECURITY_TALK:
        ...
        case DenyReason::PLAYERBOT_DENY_GEARSCORE:
            {
                int botGS = (int)GetBotAI(bot)->GetEquipGearScore(bot, false, false);
                int fromGS = (int)GetBotAI(bot)->GetEquipGearScore(from, false, false);
        ...
        case DenyReason::PLAYERBOT_DENY_NOT_LEADER:
            if (GetBotAI(bot)->GetGroupMaster() && GetBotAI(bot)->IsSafe(GetBotAI(bot)->GetGroupMaster()))
                out << "I am in a group with " << GetBotAI(bot)->GetGroupMaster()->GetName() << ". You can ask him for invite.";
            else
