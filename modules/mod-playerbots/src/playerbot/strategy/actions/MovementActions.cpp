...
bool MovementAction::MinimalMove(PlayerbotAI* ai)
{
   ...
   //Transport handling: If not on transport wait for transport and teleport on it when it's near (and cut to last transport point). If on transport wait until it is near exit and teleport to exit.
    ...
    {
        time += (nextStep->point.distance(it->point) / bot->GetSpeedInMotion()) * 1000;
    ...
    */
    ...
    if (obj->IsUnit() && sServerFacade.IsHostileTo(bot, static_cast<Unit*>(obj)))
    {
        ...
        if (stance && stance->getName() == "spread")
        {
            ...
            {
                const float distanceToStance = bot->GetDistance2d(stanceLoc.coord_x, stanceLoc.coord_y);

                if (distanceToStance > sPlayerbotAIConfig.targetPosRecalcDistance)
                {
                    return MoveTo(stanceLoc.mapId, stanceLoc.coord_x, stanceLoc.coord_y, stanceLoc.coord_z);
                }
            }
        }
    }
